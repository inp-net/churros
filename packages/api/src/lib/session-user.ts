...
export interface SessionUser
  ...
  /** Used to ensure the SessionUser we'll get from Redis actually matches the expected type: if it comes from an earlier version of the API, some includes could be missing */
  apiVersion: string;
}
...
async function getCachedSessionUser(uid: User['uid']): Promise<SessionUser | null> {
  const cachedUser: SessionUser | null = await redisClient()
  ...
  if (!('apiVersion' in cachedUser) || typeof cachedUser.apiVersion !== 'string') {
    console.warn(`User session for ${uid} is missing an API version`);
    return null;
  }

  if (cachedUser.apiVersion !== CURRENT_VERSION) {
    console.warn(`User session for ${uid} is from an older API version`);
    return null;
  }
