...
Do a \`GET\` request to \`${process.env.PUBLIC_FRONTEND_ORIGIN}/authorize?client_id=<clientId>&redirect_uri=<redirectUri>&response_type=code&state=<state>\` with:
...
Do a \`POST\` request to \`${process.env.PUBLIC_FRONTEND_ORIGIN}/token\` with a \`application/x-www-form-urlencoded\` body with the following fields:
