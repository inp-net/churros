/*
  ...
  - You are about to drop the column `answeredById` on the `Answer` table. All the data in the column will be lost.
  - A unique constraint covering the columns `[questionId,createdById]` on the table `Answer` will be added. If there are existing duplicate values, this will fail.

*/
-- DropForeignKey
ALTER TABLE "Answer" DROP CONSTRAINT "Answer_answeredById_fkey";

-- DropIndex
DROP INDEX "Answer_questionId_answeredById_key";

-- AlterTable
ALTER TABLE "Announcement" ALTER COLUMN "id" SET DEFAULT nanoid('ann:');

-- AlterTable
ALTER TABLE "Answer" DROP COLUMN "answeredById",
ADD COLUMN     "createdById" TEXT,
ALTER COLUMN "id" SET DEFAULT nanoid('answer:');
