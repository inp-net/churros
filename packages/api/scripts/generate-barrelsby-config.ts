...
${existsSync(`${modulePath}/types/index.ts`) ? `export * from './types/index.js';\n` : ''}${existsSync(`${modulePath}/resolvers/index.ts`) ? `export * from './resolvers/index.js';\n\n` : ''}`;
  ...
  // lib/env.ts needs to be imported before all others
  let libIndexContent = `/**
  * @file generated by packages/api/scripts/generate-barrelsby-config.ts
...
export * from './env.js';\n`;
  ...
  for (const file of readdirSync('src/lib')) {
    ...
    libIndexContent += `export * from './${file.slice(0, -3)}.js';\n`;
  }

  writeFileSync('src/lib/index.ts', libIndexContent);
};
