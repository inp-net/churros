...
const { frappe_annee, frappe_commentaire, frappe_document, frappe_document_tags, frappe_documentfichier, frappe_filiere, frappe_matiere, frappe_tag, portailuser_portailuser, auth_user } = Convert.toFrappeTypes(readFileSync("./frappe-dump.json").toString())
...
console.info(`- Deleted ${Object.entries(deletionCounts).map(([k,v]) => `${v} ${k}s`).join(", ")}`)
...
for (const { annee, auteur_id, creation, derniere_modif, description, matiere_id, nom, id } of frappe_document) {
    ...
    const subject = await p.subject.findUniqueOrThrow({ where: { uid: slug(frappe_matiere.find(m => m.id === matiere_id)!.nom) } , include:{minors: true}})
    const author = await p.user.findUnique({where: {
        uid: auth_user.find(u => u.id === portailuser_portailuser.find(pu => pu.user_ptr_id === auteur_id)!.user_ptr_id )!.username
    }})
    if (frappe_document_tags.map(dt => frappe_tag.find(t => t.id === dt.tag_id)).every(t => t?.ecole_id !== '2')) {
        // console.warn(`- Skipping ${nom} (${subject.uid}) because it has tags from other schools`)
        bar.increment()
        continue
    }
    const tags =  frappe_document_tags.filter(dt => dt.document_id === id).map(dt => frappe_tag.find(t => t.id === dt.tag_id)).filter(Boolean).map(t => TAG_TO_SUBJECT_AND_IS_SOLUTION[t!.nom as keyof typeof TAG_TO_SUBJECT_AND_IS_SOLUTION])

    const [type, isSolution] = tags[0] ?? [DocumentType.Miscellaneous, false]

    const uidBase = `${slug(nom)}${annee ? `-${annee}` : ''}`
    const uidNumber = await dichotomid(async n => !(await p.document.findUnique({where: {
...
for (const { auteur_id, creation, derniere_modif, document_id, message } of frappe_commentaire) {
    let author = await p.user.findUnique({where: {
        uid: auth_user.find(u => u.id === portailuser_portailuser.find(pu => pu.user_ptr_id === auteur_id)!.user_ptr_id )!.username
    }})
