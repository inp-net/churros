...
async function createMinors(major: {id: string; shortName: string}, yearTier: number, minors: Array<{name: string; shortName: string}>) {
    console.log(`* Creating minors for ${yearTier}A ${major.shortName} `)
    await Promise.all(minors.map(async ({name, shortName}) => {
...
const SN = await p.major.upsert({where:{  uid: 'sdn'}, update: {}, create:{name: "Sciences du Numérique", shortName: "SN",
  uid: 'sdn'
}})
const EEEA = await p.major.upsert({where:{  uid: 'eeea'}, update: {}, create: {name: "Électronique, Énergie Électrique et Automatique", shortName: "3EA",
  uid: 'eeea'
}})
const MFEE = await p.major.upsert({where:{  uid: 'mfee'}, update: {}, create: {name: "Mécanique, Fluides, Énergétique, Énergétique", shortName: "MF2E",
  uid: 'mfee'
}})
...
const SUBJECTS: Record<"MFEE"|"EEEA"|"SN", {
...
"Hydraulique",
"Probabilités", "Statistique", "Équation différentielles",
"Calcul Scientifique",
 ...
 "Éléments de base pour la simulation avancée",
    ...
    SN: {
        "1": {
            ...
            ].map(m => ({semester: Number.parseFloat(m.split('-')[0]!) as 1|2, name: m.split('-')[2]!, oldFrappeCodes: m.split('-')[1]?.split(' ').map(Number)})),
            FISE: [{
...
"1048-S8-Architecture des Réseaux",
                ...
                return {semester: semester === "S8" ? 2 : 1, name, oldFrappeCodes: Number.parseInt(code!, 10)}
            }),
            FISE: [
"847-S8-TAV (M)",
...
async function createSubjects(byYearTier: typeof SUBJECTS[keyof typeof SUBJECTS], major: {id: string; shortName: string}) {
...
for (let subject of subjects) {
  ...
  try {
    ...
    console.info(`\t- Created subject ${uid} (${minors.length === 0 ? '*' : minors.map(m => m.shortName).join(', ')})`)
