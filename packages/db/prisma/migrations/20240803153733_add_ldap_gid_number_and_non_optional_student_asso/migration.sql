/*
  ...
  - A unique constraint covering the columns `[ldapGidNumber]` on the table `Group` will be added. If there are existing duplicate values, this will fail.
  - Made the column `studentAssociationId` on table `Group` required. This step will fail if there are existing NULL values in that column.
...
ADD COLUMN     "ldapGidNumber" SERIAL NOT NULL,
ALTER COLUMN "id" SET DEFAULT nanoid('g:'),
ALTER COLUMN "studentAssociationId" SET NOT NULL;
...
-- CreateIndex
CREATE UNIQUE INDEX "Group_ldapGidNumber_key" ON "Group"("ldapGidNumber");
