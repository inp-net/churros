...
function documentTypeSentence(type: DocumentType, subject: string) {
    ...
    if (articles[type]) return `Ajouter ${articles[type]} ${properCasing(DISPLAY_DOCUMENT_TYPES.get(type)!)} de ${properCasing(subject)}`
    ...
    let document= {
        ...
        title: "",
        description: "",
        type: ([...DISPLAY_DOCUMENT_TYPES.keys()].find(k => $page.url.searchParams.get('type') === k) ?? 'Exam') as DocumentType,
