...
import { page } from "$app/stores";
    ...
    const minors = Object.values(Object.fromEntries(data.subjectsOfMajor.edges.flatMap(({node}) => node.minors).map(m => [m.uid, m])))

    function subjectsSorter(a: {nextExamAt?: Date|undefined}, b: {nextExamAt?: Date|undefined}) {
...
return data.subjectsOfMajor.edges.filter(({node}) => minor === undefined ? node.minors.length === 0 : node.minors.some(m => m.uid === minor.uid)).sort(subjectsSorter)
    }
