import { BookingPDFExportStore, graphql, type BookingPDFExport$result } from '$houdini';
...
graphql(`
  query BookingPDFExport($code: String!) @blocking {
    booking(code: $code) {
      id
      code
      ticket {
...
export async function GET(event) {
  const { data } = await new BookingPDFExportStore().fetch({
    event,
    variables: { code: event.params.code },
  });

  if (!data?.booking) error(404, { message: 'Code de réservation invalide' });

  event.setHeaders({
    'Content-Type': 'application/pdf',
    'Content-Disposition': `attachment; filename="reservation-churros-${data.booking.code}.pdf"`,
  });

  return new Response(await generatePDF(data.booking, event));
}

async function generatePDF(
  registration: NonNullable<BookingPDFExport$result['booking']>,
  event: RequestEvent,
): Promise<Blob> {
  ...
  const dd = {
    ...
    content: [
      ...
      },
      {
        ...
        columns: [
          {
            qr: new URL(route('/bookings/[code]', registration.code), event.url).toString(),
            width: 128,
          },
          {
            columns: [
  ...
  return new Promise((resolve, reject) => {
    ...
    pdf
      .pipe(blobStream())
      .on('finish', (blob) => {
        resolve(blob.toBlob('application/pdf'));
      })
      .on('error', (err) => {
        console.error(`Could not generate PDF for booking ${registration.code}: ${err}`);
        reject(err);
      });
    pdf.end();
  });
