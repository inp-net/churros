...
export const _articleQuery = (loggedIn: boolean) =>
  Selector('Article')({
    id: true,
    uid: true,
    title: true,
...
export const load: PageLoad = async ({ fetch, params, parent }) => {
  const { canEditGroup, token, me } = await parent();

  const { article } = await loadQuery(
    {
      article: [{ uid: params.uid, groupUid: params.group }, _articleQuery(Boolean(me))],
    },
    { fetch, token },
  );
