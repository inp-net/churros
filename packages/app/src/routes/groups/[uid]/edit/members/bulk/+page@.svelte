  ...
  return str.normalize('NFD').replaceAll(/[\u0300-\u036F]/g, '');
    ...
    async function addMember(line: string) {
        ...
        try {
            ...
            searchUsers = searchUsers.filter(u => compare(`${u.firstName} ${u.lastName}`) === compare(line))
            if (searchUsers.length === 0) 
                throw new Error(`Utilisateur·ice introuvable`)
             else if (searchUsers.length > 1) 
                throw new Error(`Plusieurs utilisateur·ice·s trouvé·e·s, utilise plutôt l'adresse e-mail ou l'@`)
            ...
            const {addGroupMember} = await $zeus.mutate({
                addGroupMember:[{
                    ...
                    title: "Membre",
                    uid
                }, {__typename: true, '...on Error':{message: true}, '...on MutationAddGroupMemberSuccess':{data: {__typename: true}}}]
            })
            if (addGroupMember.__typename === "Error") 
                throw new Error(addGroupMember.message)
    ...
    async function addMembers() {
        ...
        try {
            ...
            membersToAdd = lines.filter(l => result.some(([line, status]) => l===line && status === State.Errored)).join('\n')
        } finally {
            adding = false
        }
    }
...
<div class="content">
    <h1>
        ...
        Ajouter des membres en masse</h1>
<form on:submit|preventDefault={addMembers}>
    
    <InputLongText bind:value={membersToAdd} placeholder={"annie.versaire@etu.inp-n7.fr\nrick.astley@etu.inp-n7.fr\nines.alamaternite@gmail.com"} label="Membres" hint="Une adresse e-mail, @identifiant ou Prénom Nom de famille par ligne"></InputLongText>
...
<section class="results">
    <ul class="nobullet">
        ...
        class:danger={status === State.Errored} class:success={status === State.Success}
        >
            <span class:loading={status === State.Adding} class="status" >{#if status === State.Errored}<IconError></IconError> {:else if status === State.Success} <IconCheck></IconCheck> {:else}
                <IconLoading></IconLoading>
                {/if}</span>
                <p>
                    {line}
                    {#if uid}
                    <a href="/users/{uid}">{#if line.trim() === `@${uid}`}profil{:else}@{uid}{/if}</a>
                    {/if}
                    {#if status === State.Errored}
                    <br>
                    <div class="message">{#if !uid}Utilisateur·ice introuvable{:else}{message}{/if}</div>
                    {/if}
                </p>
        </li>
        {/each}
