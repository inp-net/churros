...
import IconDelete from "~icons/mdi/delete-outline"
    ...
    async function submit() {
        ...
        const {upsertDocument} = await $zeus.mutate({
            upsertDocument:[{
               ...
               id: data.id, 
                ...
                "...on MutationUpsertDocumentSuccess": {data: {
                    id: true,
                    subject: {uid:true, majors: {uid: true}, minors: {uid: true, yearTier: true, majors: {uid: true}}},
                    uid: true,
                }}
            }]
        })
        ...
        await Promise.all([...data.solutionPaths, ...data.paperPaths].map(async path => {
            await $zeus.mutate({
    ...
    <form on:submit|preventDefault={submit}>
        <div class="side-by-side">
            <section class="metadata">
                <h2>Infos</h2>
                <InputSelectOne label="Type de document" required options={DISPLAY_DOCUMENT_TYPES} bind:value={data.type}></InputSelectOne>
                <InputSubject label="Matière" uid={data.subject?.uid} bind:object={data.subject}></InputSubject>
                <InputText label="Titre" required bind:value={data.title}></InputText>
                <InputNumber unit="{data.schoolYear}-{data.schoolYear+1}" hint="Mettre la plus petite année. Par exemple, {new Date().getFullYear()} pour l'année {new Date().getFullYear()}-{new Date().getFullYear() + 1}" label="Année scolaire" bind:value={data.schoolYear}></InputNumber>
                ...
                <InputFile bind:inputElement={fileInputElement} label="Ajouter des fichiers" multiple bind:files={files}>
                    <ul class="new-files nobullet">
                        {#each files ? [...files] : [] as file}
                            <li class="existing-file">
                                <span class="filename">{file.name}</span>  <InputCheckbox help="Ce fichier est (ou fait partie d'un) corrigé" label="Correction" bind:value={newFilesSolutions[file.name]} ></InputCheckbox> <ButtonGhost
                                on:click={() => { files = fileListOf([...files??[]].filter(f => f.name !== file.name)) }}
                                danger help="Supprimer"><IconDelete></IconDelete></ButtonGhost> 
                            </li>
                        {/each}
                    ...
                    <li class="existing-file">
                        <span class="filename">{filepath.split('/').at(-1)}</span>
                         <InputCheckbox help="Ce fichier est (ou fait partie d'un) corrigé" label="Correction" on:change={(e) => {
                         ...
                         }} value={data.solutionPaths.includes(filepath)} ></InputCheckbox> <ButtonGhost on:click={() => {
                            filesToDelete.add(filepath)
                            data.paperPaths = data.paperPaths.filter((path) => path !== filepath)
                            data.solutionPaths = data.solutionPaths.filter((path) => path !== filepath)
                         }}  danger help="Supprimer"><IconDelete></IconDelete></ButtonGhost> </li>
                    {/each}
                    </ul>
                </InputFile>
            </section>
