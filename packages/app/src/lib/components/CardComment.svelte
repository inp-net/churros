...
<article class="comment">
    <div class="body-and-actions">
        ...
        <div class="body">
            {#if editing}
            <form on:submit|preventDefault={() => {dispatch('edit', body); editing =false}}>
                <InputLongText submitShortcut rows={Math.max(body.split('\n').length, 2)} autofocus label="" rich bind:value={body}></InputLongText>
            </form>
            {:else}
...
{/if}
                <ButtonGhost class="danger" on:click={() => {
                    dispatch('delete', id)
...
{/if}
            ...
            {#if !isSameSecond(createdAt, updatedAt ?? createdAt) } Modifié le {formatDateTime(updatedAt)}{:else}
 ...
 {#if replyingTo.inReplyToId !== id }
                 ...
                 {:else}
                 <form class="new-reply" on:submit|preventDefault={() => {
                     dispatch('reply')
                 }} >
 
 <InputLongText submitShortcut rows="2" autofocus label="" rich bind:value={replyingTo.body} placeholder={"Ajouter une réponse"}></InputLongText>
 ...
 {/if}
    ...
    .new-reply :global(> :first-child), .body form :global(> :first-child) {
        flex-grow: 1;
    }
