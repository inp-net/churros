...
export async function saveSessionToken(
  ...
  if (cookies) {
    console.log(JSON.stringify({ setting_web_cookie: token, exp: expiration?.toString() ?? '<none>' }));
    ...
    console.log(JSON.stringify({ saving_token: token, exp: expiration?.toString() ?? '<none>' }));
